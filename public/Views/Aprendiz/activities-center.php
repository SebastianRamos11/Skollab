<?php
  ...
  if (isset($_SESSION['id'])) {
    $subject_query = "SELECT * FROM materia";
    $subject_result= mysqli_query($dbConnection, $subject_query) or die(mysqli_error($dbConnection));
    $subject_array = mysqli_fetch_all($subject_result, MYSQLI_NUM);
      ...
      <div>
        <?php
          for($i = 0; $i < sizeof($course_array); $i++){
            ...
            $act = "SELECT AC.ID_Actividad FROM actividad AC JOIN ambiente_virtual A ON AC.ID_Persona = A.ID_Persona WHERE AC.ID_Ficha = $ficha AND A.ID_Materia = $subject;";
            ...
            <div class="activities-course">
              <div class="activities-course__label"><?php echo $get_program_array[0][0]; ?></div>
              <hr>
              <?php
                if (sizeof($pending) > 0){
                  ?>
                  <div class="activities">
                    <?php
                      for($j=0; $j < sizeof($pending); $j++){
                        ...
                        $activity = "SELECT AC.ID_Actividad, AC.asunto, AC.descripcion, AC.fecha, AC.fecha_limite, AC.url, A.ID_Materia FROM actividad AC JOIN ambiente_virtual A ON AC.ID_Persona = A.ID_Persona WHERE AC.ID_Ficha = $ficha AND AC.ID_Actividad = $id_activity AND A.ID_Materia = $subject;";
